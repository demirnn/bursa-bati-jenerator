...
const PortableProductDetail: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const [product, setProduct] = useState<SheetProduct | null>(null);
  const [loading, setLoading] = useState(true);
  const [activeTab, setActiveTab] = useState<'images' | 'specs' | 'docs' | 'certs'>('specs');
  ...
  return (
    <div className="min-h-screen bg-slate-50 pt-32 pb-20">
      <SEO
        title={`${product.modelName} - ${product.brand} Portatif Jeneratör`}
        description={`${product.modelName} ${product.brand} portatif jeneratör. Standby: ${product.standbyPowerKva} kVA, Prime: ${product.primePowerKva} kVA. Teknik özellikler ve dokümantasyon.`}
        canonical={`/portable-product/${id}`}
        ogImage={product.image}
        breadcrumbs={[
          { name: 'Ana Sayfa', url: '/' },
          { name: 'Ürünler', url: '/products' },
          { name: 'Portatif Jeneratörler', url: '/products/portable' },
          { name: product.modelName, url: `/portable-product/${id}` },
        ]}
        product={{
          name: product.modelName,
          description: `${product.modelName} ${product.brand} portatif jeneratör. Standby: ${product.standbyPowerKva} kVA, Prime: ${product.primePowerKva} kVA.`,
          image: product.image,
          brand: product.brand,
          category: 'Portatif Jeneratör',
          sku: product.id,
        }}
      />
          ...
          <div className="grid md:grid-cols-2">
            ...
            <div className="p-12 flex flex-col justify-center">
              ...
              <div className="flex flex-wrap gap-4">
                ...
                <div className="bg-slate-50 border border-slate-200 px-6 py-3 rounded-none">
                  <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Yakıt Tipi</div>
                  <div className="text-sm font-black text-slate-900">{product.fuelType || '-'}</div>
                </div>
                <div className="bg-slate-50 border border-slate-200 px-6 py-3 rounded-none">
          ...
          <div className="p-12">
            {activeTab === 'images' && (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {[product.image, product.secondImage, product.thirdImage].filter(Boolean).map((img, i) => (
            ...
            {activeTab === 'specs' && (
              <div className="grid md:grid-cols-2 gap-x-20 gap-y-12">
                ...
                <div>
                  <h3 className="text-lg font-black uppercase tracking-tighter mb-8 border-b-2 border-blue-600 inline-block">Alternatör & Ölçüler</h3>
                  <div className="space-y-4">
                    ...
                    <SpecRow label="Ağırlık" value={`${product.cabinetDryWeight} kg`} />
                    <SpecRow label="Yakıt Kapasitesi" value={`${product.cabinetFuelCapacity} lt`} />
                    <SpecRow label="Batarya" value={product.battery} />
                  </div>
                </div>
...
export default PortableProductDetail;
